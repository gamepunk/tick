...
from tick.core.models import Symbol, AssetType, FetchConfig, Interval, ASSET_TYPE_NAMES
...
def page_single_symbol():
    ...
    with col1:
        symbol_input = st.text_input(
            "品种代码",
            value="AAPL",
            help="例如: AAPL, sh600519, BTC-USD, GSPC, GC=F"
        )
    ...
    with col3:
        interval = st.selectbox(
            "K线周期",
            options=[i.value for i in Interval],
            index=5
        )
                ...
                if result.success:
                    ...
                    if 'rsi' in df.columns or 'macd' in df.columns:
                        ...
                        if 'macd' in df.columns:
                            ...
                            fig_macd.add_trace(go.Bar(x=df.index, y=df['macd_hist'], name='MACD Hist', marker_color=colors))
...
def page_multi_symbol():
    ...
    if st.button("🔍 对比分析", type="primary", use_container_width=True):
        ...
        if all_data:
            ...
            returns_df = returns_df.sort_values('收益数值', ascending=False).drop('收益数值', axis=1)
            ...
            with st.expander("下载合并数据"):
                ...
                for symbol, df in all_data.items():
                    ...
                    merged_data.append(df_copy[['date', 'symbol', 'open', 'high', 'low', 'close', 'volume']])
            ...
            if all_results:
                ...
                for item in all_results:
                    with st.container():
                        ...
                        with col4:
                            if st.button("选择", key=f"select_{item.get('symbol', '')}_{item.get('source', '')}"):
