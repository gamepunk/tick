...
from tick.core.models import FetchConfig, FetchResult, Symbol, CCXT_EXCHANGES, EXCHANGE_QUOTE
...
class CCXTDataSource(BaseDataSource):
    ...
    def fetch(self, config: FetchConfig) -> FetchResult:
        ...
        timeframe = interval_map.get(
            config.interval.value if hasattr(config.interval, 'value') else config.interval,
            "1d"
        )
        ...
        try:
            ...
            all_ohlcv = []
            while since < end_ts:
                ohlcv = exchange.fetch_ohlcv(ccxt_symbol, timeframe=timeframe, since=since, limit=1000)
                if not ohlcv:
                    break
    ...
    def validate_symbol(self, symbol: Symbol) -> bool:
        ...
        if s.endswith("-USD") or s.endswith("-USDT"):
            return True
        # 常见加密货币代码
        crypto_codes = {"BTC", "ETH", "SOL", "BNB", "DOGE", "XRP", "ADA", "AVAX", "DOT", "MATIC"}
    ...
    def search(self, query: str, limit: int = 10) -> list[Dict[str, str]]:
        ...
        query = query.upper()
        results = [p for p in popular if query in p["symbol"] or query in p["name"].upper()]
