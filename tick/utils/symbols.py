...
from tick.core.models import US_INDEX_CODES, AssetType, Symbol
...
def create_symbol(raw: str, asset_type: Optional[str] = None) -> Symbol:
    ...
    return Symbol(raw=raw.strip(), normalized=normalized, asset_type=at)


def parse_asset_types(
    symbols: tuple[str, ...], assets: tuple[str, ...]
    ...
    """
    if not assets:
        return {s: None for s in symbols}  # type: ignore[dict-item]
