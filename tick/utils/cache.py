...
class DataCache:
    ...
    def _make_key(self, symbol: str, start: str, end: str, interval: str) -> str:
        """生成缓存键"""
    ...
    def get(self, symbol: str, start: str, end: str, interval: str) -> Optional[pd.DataFrame]:
        """获取缓存数据"""
        key = self._make_key(symbol, start, end, interval)
    ...
    def set(self, symbol: str, start: str, end: str, interval: str, df: pd.DataFrame, ttl: Optional[int] = None):
        """设置缓存数据"""
        key = self._make_key(symbol, start, end, interval)
        ttl = ttl or self.ttl
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                    """,
                    (key, symbol, start, end, interval, data_blob, expires_at.isoformat())
                )
        except Exception:
            pass  # 缓存失败不阻止主流程
